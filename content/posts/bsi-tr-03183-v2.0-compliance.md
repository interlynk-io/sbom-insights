...
**SBOM MUST NOT contain vulnerability information.** This is one of the clearest tightenings from v1.1 to v2.0. v1.1 said an SBOM "does not contain" vulnerability data. v2.0 says it **MUST NOT**, and adds:
...
**Three-tier field system.** v1.1 had two tiers: Required and Additional. v2.0 adds a third tier:
...
**Hash algorithm changed.** v1.1 required **SHA-256**. v2.0 now requires **SHA-512** for the deployable component hash. And the field is renamed from "Hash value of the *executable* component" to "Hash value of the *deployable* component", a deliberate change to cover artifacts that are not traditionally "executable" (like container images or packages).
...
**CycloneDX minimum version bumped.** v1.1 required CycloneDX 1.4+. v2.0 requires **CycloneDX 1.5+**. SPDX minimum is now **2.2.1+** (was 2.3+).
...
## BSI TR-03183-2 v2.0 Required Fields
...
The rationale is identical to v1.1: a machine-actionable contact point so automated tooling can reach the responsible party without human intervention. v2.0 adds explicit examples of acceptable URLs (creator's home page or project web page), making it clearer what a fallback URL should look like.
...
Same as v1.1. Ties the SBOM to a specific point in time for vulnerability correlation and audit trails.
...
v2.0 extends this: dependencies now also include components that are *contained within* this component (e.g., libraries statically linked into a binary, or files bundled inside an archive). Not just what it depends on at runtime, but what it physically contains.
...
Why does this matter? An archive may contain sub-components that need to be individually enumerated in the SBOM. Declaring archive status signals to consumers that this component may itself contain further nested components.
...
This property helps tooling decide whether the contents of an archive can be automatically inventoried or require manual analysis.
...
## BSI TR-03183-2 v2.0 Additional Fields
...
## BSI TR-03183-2 v2.0 Optional Fields
...
## BSI TR-03183-2 v2.0 Field Summary Table
...
```bash
sbomqs score --profile bsi-v2.0 ../sbomqs/testdata/bsi/cdx/validFullyCompliantBsiV20.cdx16.json

SBOM Quality Score: 10.0/10.0     Grade: A    Components: 2      EngineVersion: 6    File: ../sbomqs/testdata/bsi/cdx/validFullyCompliantBsiV20.cdx16.json


+---------------------+-----------------------------+------------------------+---------------------------------------+
|       PROFILE       |           FEATURE           |         STATUS         |                 DESC                  |
+---------------------+-----------------------------+------------------------+---------------------------------------+
| BSI TR-03183-2 v2.0 | sbom_creator                | 10.0/10.0              | SBOM creator contact(email)           |
...
|                     |                             |                        | declared and structurally complete.   |
+                     +-----------------------------+------------------------+---------------------------------------+
|                     | comp_license                | 10.0/10.0              | associated licence declared for       |
|                     |                             |                        | all components                        |
+                     +-----------------------------+------------------------+---------------------------------------+
|                     | comp_hash                   | 10.0/10.0              | deployable component declares a       |
|                     |                             |                        | valid SHA-512 hash.                   |
+                     +-----------------------------+------------------------+---------------------------------------+
|                     | comp_executable_property    | 10.0/10.0              | executable property declared for      |
|                     |                             |                        | all components                        |
+                     +-----------------------------+------------------------+---------------------------------------+
|                     | comp_archive_property       | 10.0/10.0              | archive property declared for         |
|                     |                             |                        | all components                        |
+                     +-----------------------------+------------------------+---------------------------------------+
|                     | comp_structured_property    | 10.0/10.0              | structured property declared for      |
|                     |                             |                        | all components                        |
+                     +-----------------------------+------------------------+---------------------------------------+
...
|                     |                             |                        | all components.                       |
+                     +-----------------------------+------------------------+---------------------------------------+
|                     | comp_deployable_url         | 10.0/10.0 (additional) | Deployable URI declared for           |
|                     |                             |                        | all components.                       |
+                     +-----------------------------+------------------------+---------------------------------------+
...
Summary:
Required Fields   : 12/12 compliant
...
The `(additional)` label in the STATUS column clearly separates required from conditional compliance. For most real-world SBOMs, the biggest gaps will appear in the new required fields: filename, executable property, archive property, and structured property are rarely populated by standard SBOM generators today.
...
In the next post of this series, we'll look at **OpenChain Telco (OCT)**, a profile born from the telecom sector that focuses on open-source licence transparency and SBOM quality for high-availability infrastructure.
...
- [Part 3: BSI TR-03183-2 v1.1](https://sbom-insights.dev/posts/ntia-minimum-elements-2021-compliance/)
